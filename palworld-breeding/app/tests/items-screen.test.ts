...
describe('the app says what changed when the data was refreshed (IL98)', () => {
  it('the change list is generated, not written by hand', () => {
    const gen = readFileSync(
      join(__dirname, '../../tools/gen_data_changes.py'), 'utf8');
    expect(gen).toContain('def summarise(');
    // it compares two files we hold and names nothing else
    expect(gen).toContain('added = sorted(set(cf) - set(pf))');
    expect(gen).toContain('json.dumps(pf[iid].get(k), sort_keys=True)');
    const out = readFileSync(
      join(__dirname, '../../mobile/src/data/dataChanges.g.ts'), 'utf8');
    expect(out).toContain('GENERATED by tools/gen_data_changes.py');
  });

  it('what it reports for today matches what actually changed', () => {
    const out = readFileSync(
      join(__dirname, '../../mobile/src/data/dataChanges.g.ts'), 'utf8');
    // the five Grappling Gun tiers gained facts when their page was
    // recovered, and the three stat fruits plus Seafood Pasta gained an
    // effect from a label nothing had mapped
    expect(out).toContain('5 items gained facts they had none of — Grappling Gun');
    expect(out).toContain('4 effects changed');
    for (const n of ['Life Fruit', 'Power Fruit', 'Stout Fruit']) {
      expect(out).toContain(n);
      expect(effectNumber(itemIdByName(n)!, 'Nutrition')).toBe(1);
    }
    // and those items really do carry the new effects now
    expect(effectNumber(itemIdByName('Life Fruit')!, 'Health IV')).toBe(10);
    expect(ITEM_FACTS[itemIdByName('Grappling Gun')!]?.tech?.level).toBe(12);
  });

  it('the names are said once even when five tiers share one', () => {
    const out = readFileSync(
      join(__dirname, '../../mobile/src/data/dataChanges.g.ts'), 'utf8');
    expect(out).not.toContain('Grappling Gun, Grappling Gun');
  });

  it('the Reference screen shows it, and the stamp leads there', () => {
    const ref = readFileSync(
      join(__dirname, '../../mobile/src/screens/ReferenceScreen.tsx'), 'utf8');
    expect(ref).toContain('What changed in the data');
    expect(ref).toContain('DATA_CHANGES.lines.map');
    const kit = readFileSync(
      join(__dirname, '../../mobile/src/ui/kit.tsx'), 'utf8');
