...
import { DATA_CHANGES } from '../data/dataChanges.g';
...
/** "2026-08-20" -> "20 Aug 2026" — the same shape the data stamp uses. */
...
export function ReferenceScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
      ...
      {/* IL98: the data stamp on every screen leads here, and until now
          "where these come from" answered only WHERE — never WHAT MOVED
          when the numbers were last refreshed. Generated by diffing the
          shipped item data against the build before it; nothing on this
          card is written by hand. */}
      {DATA_CHANGES.lines.length > 0 && (
        <Card style={{ marginBottom: 14 }}>
          <Text style={s.h3}>What changed in the data</Text>
          <Text style={[s.body, { fontSize: 12, color: T.muted, marginTop: 2 }]}>
            Refreshed {spokenDay(DATA_CHANGES.when)}
            {DATA_CHANGES.label ? ` · ${DATA_CHANGES.label}` : ''}
          </Text>
          <View style={{ marginTop: 8, gap: 5 }}>
            {DATA_CHANGES.lines.map((ln) => (
              <Text key={ln} style={[s.body, { fontSize: 12.5 }]}>· {ln}</Text>
            ))}
          </View>
        </Card>
      )}
