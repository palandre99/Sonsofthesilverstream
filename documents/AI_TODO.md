...
- [x] IL12 2026-08-19: accessories say what they grant — 80 of 81 pages
      ...
      Lv. 3"), on cards and in shares, no-underscore pin catalogue-wide.
      Eye-verified on Life Pendant. 811 green, published.
...
### M50 — THE 20:0x SCREENSHOT ROUND: four fixes, shipped (14e3d61, published, both channels confirmed)
He sent four phone reports with screenshots inside forty minutes; all four
are live:
1. "Icons insanely low quality and pixelated ... only high quality when max
   zoomed out" — HIS SCREENSHOTS CONTAINED THE PROOF: place names (screen
   space) razor-sharp beside pins (inside the map transform) that blurred
   deeper with every zoom level. iOS rasterises the transformed subtree at
   its intermediate counter-scaled size and magnifies the raster, so
   net-scale-1 still renders soft — the browser can never show it (JS-thread
   worklets, F35). Pins now render EXACTLY like names: MarkerPin, screen
   space, per-marker worklet (the budget M28 already spent). CounterScaled
   and the second transformed container are gone.
2. "Still want to zoom in closer" — OVERZOOM 3 -> 5; MAX_ZOOM (14x floor)
   is the binding cap on a dpr-3 phone (~46% deeper). Ground past ~3.2x
   magnifies by design; the overlay stays crisp all the way down.
3. "Location names move sometimes when I move around the map" — the edge-
   slide clamp (wide labels pulled inside the frame) detached names from
   terrain during pans. REMOVED: labels are glued to their anchor and clip
   at the frame edge like every real map. ScreenPin deleted; labels ride
   MarkerPin; halfWidth is gone from both trees. The old "dragged in from
   off screen" fault class is structurally impossible now.
4. "Bosses should show lvl also in the info when tapped" — alpha cards say
   "Level N" from the datamined per-spot AlphaSpot.lv, matched by region
   AND position (a species with spots on both islands can never show the
   wrong one). Verified live: Anubis reads Level 55.
ZOOM IS FIXED AND SACRED — "Zoom is now working so don't break it" (CEO).
The transform/gesture code is UNTOUCHED in this round and stays that way
without a NEW derived fault plus his report.
PUBLISH-GUARD JUDGMENT, recorded: publish.js refused over a dirty
mobile/package-lock.json. I did NOT bypass the guard with raw eas-cli —
the guard is law. Resolution: the dirt was the 6-line mechanical echo of
the goals lane's ALREADY-COMMITTED 1.1.0 bump (c891d59), regenerated by
any npm touch; committed as housekeeping, guard cleared, published clean.
